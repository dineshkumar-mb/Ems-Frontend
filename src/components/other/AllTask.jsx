...
const AllTask = () => {
  ...
  return (
    <div className='bg-[#1c1c1c] p-4 sm:p-5 rounded mt-4 sm:mt-5'>
      <div className='overflow-x-auto'>
        <div className='min-w-[600px]'>
          <div className='bg-red-400 mb-2 py-2 px-3 sm:px-4 flex justify-between rounded'>
            <h2 className='text-sm sm:text-base md:text-lg font-medium w-1/5'>Employee Name</h2>
            <h3 className='text-sm sm:text-base md:text-lg font-medium w-1/5'>New Task</h3>
            <h5 className='text-sm sm:text-base md:text-lg font-medium w-1/5'>Active Task</h5>
            <h5 className='text-sm sm:text-base md:text-lg font-medium w-1/5'>Completed</h5>
            <h5 className='text-sm sm:text-base md:text-lg font-medium w-1/5'>Failed</h5>
            <h5 className='text-sm sm:text-base md:text-lg font-medium w-1/5'>Action</h5>
          </div>
          <div className=''>
            {userData && userData.map(function (elem, idx) {
              return <div key={idx} className='border-2 border-emerald-500 mb-2 py-2 px-3 sm:px-4 flex justify-between rounded'>
                <h2 className='text-sm sm:text-base md:text-lg font-medium w-1/5'>{elem.firstName}</h2>
                <h3 className='text-sm sm:text-base md:text-lg font-medium w-1/5 text-blue-400'>{elem.taskCounts.newTask}</h3>
                <h5 className='text-sm sm:text-base md:text-lg font-medium w-1/5 text-yellow-400'>{elem.taskCounts.active}</h5>
                <h5 className='text-sm sm:text-base md:text-lg font-medium w-1/5 text-white'>{elem.taskCounts.completed}</h5>
                <h5 className='text-sm sm:text-base md:text-lg font-medium w-1/5 text-red-600'>{elem.taskCounts.failed}</h5>
                <div className='w-1/5'>
                  <button onClick={() => handleDelete(elem._id)} className='bg-red-600 py-1 px-2 rounded text-xs sm:text-sm'>Delete</button>
                </div>
              </div>
            })}
          </div>
